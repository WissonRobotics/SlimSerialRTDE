...
void SlimSerialRTDE::SlimSerialRTDEImpl::addLoggingFile(std::string logFileName,std::string logFileLevel){
 	if(logFileName.size()>0){
		...
		else if (logFileLevel == "warn"  ||logFileLevel == "Warn" ||logFileLevel == "WARN" || logFileLevel == "warning" || logFileLevel == "Warning" || logFileLevel == "WARNING" ) {
...
inline uint8_t SlimSerialRTDE::SlimSerialRTDEImpl::getFrameType() {
	return _frameType;
}
...
WS_STATUS SlimSerialRTDE::SlimSerialRTDEImpl::transmitReceiveFrame(std::vector<uint8_t> const& txframe, uint32_t timeoutMS) {
	return transmitReceiveFrame((uint8_t *)txframe.data(),txframe.size(),timeoutMS);
}


WS_STATUS SlimSerialRTDE::SlimSerialRTDEImpl::spinWait(std::function<bool()> &&waitCondition,int timeoutMS,int intervalMS) {
...
WS_STATUS SlimSerialRTDE::SlimSerialRTDEImpl::transmitReceiveFrame(uint8_t *pData,uint16_t datasize, uint32_t timeoutMS) {
...
uint32_t SlimSerialRTDE::SlimSerialRTDEImpl::readBuffer(uint8_t* pDes, uint32_t nBytes, uint32_t timeoutMS) {
	...
	if (readN < nBytes) {
		...
		auto const timeoutPoint = std::chrono::steady_clock::now() + std::chrono::milliseconds(timeoutMS);
		if (readBufferCV.wait_until(lock_, timeoutPoint, [&]() {return circularBuffer.availableBytes() >= remainingBytes; })) {
...
//this function will decoding all the available bytes in the rx buffer, calling the frame callback in sequence, until available bytes less than a frame is left.
WS_STATUS SlimSerialRTDE::SlimSerialRTDEImpl::frameParser() {
	...
	while(true){
		...
		if(
		((_frameType == SLIMSERIAL_FRAME_TYPE_1_NUM)&&remainingBytes<7) ||
...
std::vector<uint8_t> SlimSerialRTDE::SlimSerialRTDEImpl::assembleTxFrameWithAddress(uint8_t des, uint8_t fcode, std::vector<uint8_t> const& payload) {
	if(payload.size()>0){
...
std::vector<uint8_t> SlimSerialRTDE::SlimSerialRTDEImpl::assembleTxFrameWithAddress(uint8_t des, uint8_t fcode, uint8_t *pData,uint16_t datasize) {
...
SlimSerialRTDE::SlimSerialRTDE(std::string logFileName,std::string logFileLevel):pimpl_(std::make_unique<SlimSerialRTDEImpl>(logFileName,logFileLevel)){
...
std::vector<uint8_t> SlimSerialRTDE::assembleTxFrameWithAddress(uint8_t des, uint8_t fcode, std::vector<uint8_t> const& payload) {
	return pimpl_->assembleTxFrameWithAddress(des, fcode, payload);
}
std::vector<uint8_t> SlimSerialRTDE::assembleTxFrameWithAddress(uint8_t des, uint8_t fcode, uint8_t *pData,uint16_t datasize) {
...
void SlimSerialRTDE::transmitFrameAsync(uint8_t *pData,uint16_t datasize){
	pimpl_->transmit(pData,datasize);
}
...
void SlimSerialRTDE::setFrameLengthFilter(uint16_t _lengthFilterMax) {
	pimpl_->lengthFilterMax = _lengthFilterMax;
}

void SlimSerialRTDE::toggleFuncodeFilter(bool filterOn) {
	pimpl_->funcodeFilterOn = filterOn;
}
...
uint8_t SlimSerialRTDE::getHeader(uint8_t index) {
	return pimpl_->default_headers[index];
}
void SlimSerialRTDE::setTxAddress(uint8_t srcAddress) {
	pimpl_->default_address = srcAddress;
}
 uint8_t SlimSerialRTDE::getTxAddress() {
	return pimpl_->default_address;
}
uint8_t SlimSerialRTDE::getFrameType() {
	return pimpl_->getFrameType();
}
void SlimSerialRTDE::setFrameType(uint8_t ftype) {
	pimpl_->setFrameType(ftype);
}
 
void SlimSerialRTDE::addCRC(uint8_t* pData, uint16_t bytesIncludingHeaders) {
	pimpl_->addCRC(pData, bytesIncludingHeaders);
}
uint16_t SlimSerialRTDE::calculateCRC(uint8_t* data, uint16_t datasize) {
	return pimpl_->calculateCRC(data,datasize);
}

std::string SlimSerialRTDE::toHexString(std::vector<uint8_t>& data) {
	return pimpl_->toHexString(data);
}
std::string SlimSerialRTDE::toHexString(uint8_t *pdata,uint32_t databytes) {
	return pimpl_->toHexString(pdata,databytes);
}

uint32_t SlimSerialRTDE::availableBytes() {
...
uint64_t SlimSerialRTDE::getLastTxRxFrameTime() {
	return pimpl_->m_lastTxRxTime;
}

uint64_t SlimSerialRTDE::getTimeUTC() {
	return  pimpl_->getTimeUTC();
}

uint64_t SlimSerialRTDE::getTicm_tic_frameStart() {
	return pimpl_->m_tic_frameStart;
}

uint64_t SlimSerialRTDE::getTicm_tic_rxDataCallback() {
	return pimpl_->m_tic_rxDataCallback;
}
uint64_t SlimSerialRTDE::getTicm_tic_frameParser() {
	return pimpl_->m_tic_frameParser;
}
uint64_t SlimSerialRTDE::getTicm_tic_frameComplete() {
	return pimpl_->m_tic_frameComplete;
}
uint64_t SlimSerialRTDE::getTicm_tic_frameParserEnd() {
	return pimpl_->m_tic_frameParserEnd;
}

void SlimSerialRTDE::setServer(bool isServer){
	pimpl_->m_isServer = isServer;
}