...
class SlimArm_Hardware_Simulator :public SlimSerialRTDE {
public:
	std::unique_ptr<std::jthread> internalDynamicsThread;
	SlimArm_Hardware_Simulator() {
		addRxFrameCallback(std::bind(&SlimArm_Hardware_Simulator::frameCallback, this, std::placeholders::_1,std::placeholders::_2));
	...
	void disconnect(){
		stopInternalDynamics();
		SlimSerialRTDE::disconnect();
	}

	WS_STATUS  connect(std::string portname, uint32_t baudrate)
	{
		auto ret = SlimSerialRTDE::connect(portname,baudrate,false);
		startInternalDynamics();
		return ret;
	}


	void startInternalDynamics(){
		stopInternalDynamics();
			...
			{ 
				...
				{	
					if(m_enableInternalDynamics){
						...
						dataInternal.pressures.Ppitch_inner_Down += (dataCmd.PitchCmd-dataInternal.Pitch)>=10?-10:((dataCmd.PitchCmd-dataInternal.Pitch)<=-10?10:0);
						dataInternal.pressures.Ppitch_outer_Down += (dataCmd.PitchCmd-dataInternal.Pitch)>=10?10:((dataCmd.PitchCmd-dataInternal.Pitch)<=-10?-10:0);
						dataInternal.pressures.Ppitch_inner_Up += (dataCmd.PitchCmd-dataInternal.Pitch)>=10?-10:((dataCmd.PitchCmd-dataInternal.Pitch)<=-10?10:0);
						dataInternal.pressures.Ppitch_outer_Up += (dataCmd.PitchCmd-dataInternal.Pitch)>=10?10:((dataCmd.PitchCmd-dataInternal.Pitch)<=-10?-10:0);
						#endif
						...
						if(dataInternal.pressures.Plock > 700){
							dataInternal.IOFlags.fastener = 1; 
						}
						else if(dataInternal.pressures.Plock < -500){
							dataInternal.IOFlags.fastener = 0;
						}

						dataInternal.IOFlags.gun_status_0 = 1;
						dataInternal.IOFlags.gun_status_1 = 1;

 						dataInternal.lasers[0] = (int)((dataInternal.pressures.Pguide-(-650.0))/(950.0-(-650.0))*(161-90)+90); 
						dataInternal.lasers[1] = (int)((dataInternal.pressures.Pelongate-(-550.0))/(400.0-(-550.0))*(220-70)+70);
 

						
						// LOG_F(INFO,"p =[%d %d], laser=[%d %d]",dataInternal.pressures.Pelongate,dataInternal.pressures.Pguide,dataInternal.lasers[0],dataInternal.lasers[1]);
					}
	...
	void stopInternalDynamics(){
		if(internalDynamicsThread){
			internalDynamicsThread->request_stop();
			if(internalDynamicsThread->joinable()){
				internalDynamicsThread->join();
			}
			internalDynamicsThread.reset();
		}
	}
	...
	float pitchangle;
	bool m_enableInternalDynamics = true;
};
